...
t <- kable(aquifer_table, format = "latex", escape = FALSE) %>%
  ...
  column_spec(1, width = "5cm", color = "white",
              background = "tableblue") %>%  # Can't use border_left = TRUE, as there's a bug in kableExtra right now
...
\begin{tikzpicture}
  ...
  \fill[blue] (current page.north west) rectangle ([yshift = -2.5cm]current page.north east);
  \fill[gold] ([yshift = -2.5cm]current page.north west) rectangle ([yshift = -2.65cm]current page.north east);

  `r  # Title and Aquifer number`
  \node[below = 0.6cm of current page.north, white, font = \Huge\bf] (num) {Aquifer \#`r aq_num`};
  \node[below = 0.15cm of num, white, font = \Large\bf] {`r title`};

  `r # Map figure`
  \node[below = 3cm of current page.north] (map) {\includegraphics{`r figures_p1$map`}};
