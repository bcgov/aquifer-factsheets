...
t <- kable(aquifer_table, format = "latex", escape = FALSE, booktabs = TRUE, linesep = "") %>%
  ...
  add_footnote(label = "* min - max", escape = FALSE, notation = "none") %>%
...
t <- stringr::str_replace(t, "max\\}\\\\\\\\", "max\\}\\\\\\\\\\[-0.3em\\]")
...
\begin{tikzpicture}
  ...
  `r # Boxplots`
  \node[below = 0.25cm of map.south east, anchor = north east] (third) {\includegraphics[width = 0.175\paperwidth]{`r figures_p1$well_depth`}};
  \node[left = 0cm of third.north west, anchor = north east] (second) {\includegraphics[width = 0.175\paperwidth]{`r figures_p1$water_depth`}};
  \node[left = 0cm of second.north west, anchor = north east] (first) {\includegraphics[width = 0.175\paperwidth]{`r figures_p1$well_yield`}};
