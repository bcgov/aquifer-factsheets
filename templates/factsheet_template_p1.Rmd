...
\begin{tikzpicture}
  ...
  \node[below = 0.5cm of current page.north, white, font = \Huge\bf] (num) {Aquifer \#`r aq_num`};
  ...
  \node[below = 0.35cm of map.south west, anchor = north west, align = left, text width = 7.5cm] (desc) {`r desc`};
 ...
 \node[above right = 0.65cm and 0.8cm of current page.south west, align = left, font = \scriptsize] {Detailed methods for all figures are described in the companion document (\link{Aquifer Factsheet - Companion Document.pdf}{Aquifer Factsheet - Companion Document.pdf}).};
 
 `r # Add link to current factsheet`
 \node[above right = 0.25cm and 0.8cm of current page.south west, align = left, font = \scriptsize] {Factsheet generated: `r as.character(Sys.Date())`. Available from: \link{`r paste0("https://s3.ca-central-1.amazonaws.com/aquifer-docs/00000/", sprintf("%05d", aq_num), "\\_Aquifer\\_Factsheet.pdf")`}{`r paste0("https://s3.ca-central-1.amazonaws.com/aquifer-docs/00000/", sprintf("%05d", aq_num), "\\_Aquifer\\_Factsheet.pdf")`}. };
