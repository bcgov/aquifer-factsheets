...
```{r Aquifer Summary Table, results = 'asis', out.width = "35%"}
...
t <- kable(aquifer_table, format = "latex", escape = FALSE) %>%
  kable_styling(full_width = FALSE, position = "left", font_size = 9) %>%
  column_spec(1, width = "5cm", color = "white",
              background = "blue") %>%  # Can't use border_left = TRUE, as there's a bug in kableExtra right now
  column_spec(2, width = "2.5cm", border_right = TRUE)
...
\begin{tikzpicture}
  ...
  \node[below = 1cm of current page.north, white, font = \Huge\bf] (num) {Aquifer \#`r aq_num`};
  \node[below = 0.25cm of num, white, font = \Large\bf] {`r title`};

  `r # Map figure`
  \node[below = 3.5cm of current page.north] (map) {\includegraphics{`r figures_p1$map`}};
  
  `r # Boxplots`
  \node[below = 0.3cm of map.south east, anchor = north east] (third) {\includegraphics[width = 0.18\paperwidth]{`r figures_p1$well_depth`}};
