...
fmt_props <- function(x_min, x_max, x_n, units) {
  
  if(is.na(x_min)) {
    x <- NA_character_
  } else {
    if(x_n == 1) {
      x <- format(x_min, digits = 2, scientific = TRUE)
    } else {
      x <- paste0(format(x_min, digits = 2, scientific = TRUE),
                  " - ", 
                  format(x_max, digits = 2, scientific = TRUE))
    }
...
aquifer <- aquifer %>%
  ...
  select(`Region` = region, 
         ...
         `Hydraulic~Conductivity \\footnotesize{(min-max)}` = conductivity,
         `Transmissivity \\footnotesize{(min-max)}` = transmissivity,
         `Storativity \\footnotesize{(min-max)}` = storativity,
  ...
  mutate(across(contains("min-max"), .fns = ~if_else(is.na(.), "no data available", .))) %>%
