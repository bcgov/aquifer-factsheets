...
mapping_report <- paste0(
  "https://s3.ca-central-1.amazonaws.com/aquifer-docs/", # Main site
  sprintf("%05d", round_any(aq_num, 100, f = floor)),    # Folder round to lower 100
  "/AQ_",                                                # Prefix
  sprintf("%05d", aq_num),                               # Aq number, 5 digits
...
if(nrow(ow) > 0) {
  ...
  piper_text <- read_excel(here("data/piper_text.xlsx"), sheet = 1) %>%
    rename_all(tolower) %>%
    filter(hydrogeochemistry != "Do not publish") %>% # Omit bad plots
  ...
  figures_p2 <- tibble(a = rep(aq_num_ch, 3*nrow(ow)), ow = rep(ow$ow_ch, 3), 
    ...
    mutate(fig = case_when(
      ...
      exists = if_else(type == "piper" & !as.numeric(.data$ow) %in% .env$piper_text$obs_well, FALSE, exists),
