...
logo <- here("figures/BCID_H_cmyk_rev.pdf")

aq_num_ch <- sprintf("%04d", aq_num)

aquifer <- read_csv(here("out/aquifer_table.csv")) %>%
  filter(aquifer_id %in% aq_num) %>%
  mutate(across(where(is.character), 
                ~str_replace_all(., c("\\&" = "\\\\&", "\\#" = "\\\\#")))) # Escape & symbols
...
ow <- read_csv(here("out/aquifer_ow.csv")) %>%
  filter(aquifer_id %in% aq_num) %>%
...
wells_db <- read_csv(here("out/wells_table.csv")) %>%
  filter(aquifer_id %in% aq_num)
...
fmt_props <- function(x_min, x_max, x_n, units, digits = 3) {
  if_else(!is.na(x_min),
          paste0(format(round(x_min, digits), nsmall = digits),
                 " - ", 
                 format(round(x_max, digits), nsmall = digits),
                 " ", units, " (n=", x_n, ")"),
          NA_character_)
}
...
aquifer <- aquifer %>%
  mutate(
    ow = if_else(nrow(ow) > 0, paste0(ow$format, collapse = ", "), "None"),
    size_km2 = replace(size_km2, 
                       size_km2 != "Unknown",
                       paste(size_km2, "km\\textsuperscript{2}")),
    conductivity = fmt_props(conductivity_min, conductivity_max, conductivity_n,
                             units = "m/s"),
    transmissivity = fmt_props(transmissivity_min, 
                               transmissivity_max, transmissivity_n,
                               units = "m\\textsuperscript{2}/s"),
    storativity = fmt_props(storativity_min, 
                            storativity_max, storativity_n,
                            units = "units"),
    n_licences = replace(n_licences, n_licences == 0, "Unknown"),
    hydraulic_connectivity = if_else(
      hydraulic_connectivity == "Not Likely", 
      "\\link{https://a100.gov.bc.ca/pub/acat/documents/r50832/HydraulicConnectMW3_1474311684426_4310694949.pdf}{see guidance document}", 
  ...
  select(`Region` = region, 
         ...
         `Hydraulic~Conductivity (min-max)` = conductivity,
         `Transmissivity (min-max)` = transmissivity,
         `Storativity (min-max)` = storativity,
  ...
  mutate(across(everything(), .fns = ~if_else(is.na(.), "Unknown", .)))
...
aquifer <- aquifer %>%
  rename(`\\link{http://www.bclaws.ca/civix/document/id/complete/statreg/38_2016}{Water District}` = `Water District`,
         ...
         `\\link{http://a100.gov.bc.ca/appsdata/acat/documents/r50832/HydraulicConnectMW3_1474311684426_4310694949.pdf}{Hydraulic~Connection}` = `Hydraulic~Connection`)
...
# Page 1
figures_p1$map <- list.files(
  here("figures/maps/"), 
  pattern = paste0("Aquifer_Map_", aq_num_ch, ".pdf"), full.names = TRUE)
figures_p1$water_depth <- list.files(
  here("out/boxplots/"), 
  pattern = paste0("water_depth_", aq_num_ch, ".jpg"), full.names = TRUE)
figures_p1$well_yield <- list.files(
  here("out/boxplots/"), 
  pattern = paste0("yield_", aq_num_ch, ".jpg"), full.names = TRUE)
figures_p1$well_depth <- list.files(
  here("out/boxplots/"),
  pattern = paste0("well_depth_", aq_num_ch, ".jpg"), full.names = TRUE)

# Replace empty figures with NA plots
#if(length(figures_p1$map) == 0) figures_p1$map <- here("figures/maps/NA.png")
if(length(figures_p1$water_depth) == 0) figures_p1$water_depth <- here("out/boxplots/water_depth_NA.jpg")
if(length(figures_p1$well_yield) == 0) figures_p1$well_yield <- here("out/boxplots/yield_NA.jpg")
if(length(figures_p1$well_depth) == 0) figures_p1$well_depth <- here("out/boxplots/well_depth_NA.jpg")
...
if(nrow(ow) > 0) {
  figures_p2 <- tibble(a = rep(aq_num_ch, 3*nrow(ow)), ow = rep(ow$ow_ch, 3), 
    ...
    mutate(fig = case_when(
      type == "combo" ~ here(paste0("out/gwl/combo_", a, "_OW", .data$ow, ".png")),
      type == "trend" ~ here(paste0("out/trends/trends_", a, "_OW", .data$ow, ".png")),
      type == "piper" ~ here(paste0("figures/piperplots_trimmed/Piperplot_", a, "_OW", .data$ow, ".jpg"))),
      exists = file.exists(fig),
      fig = replace(fig, !exists, here(paste0("figures/na/figure_missing_", type[!exists], ".png")))) %>%
    group_by(.data$ow) %>%
  ...
  piper_text <- read_excel(here("data/piper_text.xlsx"), sheet = 1) %>%
    rename_all(tolower) %>%
    mutate(hydrogeochemistry = paste0(
      hydrogeochemistry,
...
# Page 3
extra <- read_csv(here("out/extra_page_images.csv"), show_col_types = FALSE) %>%
  filter(aquifer_ids %in% !!aq_num) %>%
  left_join(read_excel(here("data/extra_page_index.xlsx")), by = "type") %>%
  arrange(order, image) %>%
...
for(i in seq(1, nrow(extra), 2)) {
  e <- slice(extra, i:(i+1)) %>%
    mutate(image = here("figures/extra", image))
  cat("\\newpage")
  cat(knit_child(p3))
}
```
