...
logo <- here(fs::path(f["inputs_figures"], "BCID_H_cmyk_rev.pdf"))
...
for (o in figs_p2$ow) {
  # Page details
  f2 <- dplyr::filter(figs_p2, .data$ow == o)
...
```{r create_page3s, eval = nrow(figs_p3) > 0 & params$pages >= 3, results = "asis", message = FALSE}

for(i in unique(figs_p3$p3_n)) {
  # Page Details
  f <- dplyr::filter(figs_p3, p3_n == i)
